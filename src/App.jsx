...
function ErrorBoundary({ children }) {
  ...
  if (err) return (
    <div style={{padding:20, background:'#0B1020', color:'#fff', minHeight:'100vh'}}>
      <h2 style={{marginTop:0}}>Hata</h2>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error } = await supabase.from(TABLE).select('*').eq(cols.id, selectedId).single();
      if (ignore) return;
  ...
  const report = useMemo(() => {
    ...
    return { total, delivered, remaining, pct, statusCounts, tipStats, breakdown };
  }, [units, cols]);
        ...
        <main style={styles.main}>
          ...
          {cols && !cols.tip && (
            <div style={{ ...styles.warn, marginBottom: 8 }}>
              Not: <code>tip/kategori/tur/type</code> kolonu yok. “Konut/Ticari” blok/no kuralından hesaplanıyor.
            </div>
          )}
